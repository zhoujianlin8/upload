...
function verifyNickname(id) {
	var testStr = /^[a-zA-Z0-9\u4E00-\u9FA5]{2,16}$/;
	var val = $('#'+id).val();
	if(verifyIsNull(val)) {//验证为空
		showValidResult(id, 'error', '昵称不能为空');
		return false;
	}
	if(testStr.test(val) && getLen(val) >=4 && getLen(val) <16) {
		showValidResult(id, 'success', '');
		return true;	
	} else {
...
function verifyPassword(id, safeLevelId) {
	var valPattern = /^[A-Za-z@#!$%^*&+_-].*[0-9]|[0-9].*[A-Za-z@#!$%^*&+_-]$/;
	var val = $('#'+id).val();
	...
	if(val.length<8 || val.length>40 ){
		showValidResult(id, 'error', '密码长度8-40位');
		return false;
	}
    if(/^[a-zA-Z]{8,40}$/.test(val)){
        showValidResult(id, 'error', '密码不能全为字母');
        return false;
    }
    if(/^[0-9]{8,40}$/.test(val)){
        showValidResult(id, 'error', '密码不能全为数字');
        return false;
    }
    if(/^[@#!$%^*&+_-]{8,40}$/.test(val)){
        showValidResult(id, 'error', '密码不能全为字符');
        return false;
    }
